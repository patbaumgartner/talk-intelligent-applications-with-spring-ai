...
public class WeatherService implements Function<WeatherService.Request, WeatherService.Response> {
	...
	public Response apply(Request request) {
		...
		if (request.location().contains("Stockholm")) {
			temp = -2;
	...
	public record Request(
			@JsonProperty(required = true, value = "location") @JsonPropertyDescription("The city and country like Zurich, Switzerland") String location,
			...
			@JsonProperty(required = true, value = "unit") @JsonPropertyDescription("Local unit to measure air temperature like F for Fahrenheit countries with the imperial system and C for degree Celcius in countries with metric system.") Unit unit) {
	}
